...
key <- "35587aff2e52d96d0c22ac617d3713db90cb7aa7"
census_api_key( key )
...
get_acs( geography = "tract", 
  ...
  dplyr::rename( POP = estimate )  ## there was an error: Error in rename(., POP = estimate) : unused argument (POP = estimate)// so I added "dplyr::" to rename()
...
atl.sp$pop.w <- atl.sp$POP / 9000 # max(msp.sp$POP)   # standardizes it to max of 1.5
...
atl <- merge( atl_dorling, d, by.x="GEOID", by.y="tractid", all.x=T )
...
```{r, echo=F}
## add more when I know the dorling cartogram works as an exportable file
data.dictionary <- 
...
# atl_dorling <- st_as_sf(atl_dorling)  ## this did not fix it!!
#atl_sp <- as_Spatial( atl )  ## this did it! I kept getting random "no polygon found" or "CRS()" wouldn't work
# the above comments are likely from when i had the incorrect object selected, which is why it kept showing older code line as the shape file, plotting the "dorling cartogram" with counties instead of circles
...
# write to file 
geojson_write( atl, file="atl_dorling_sp_3.geojson", geometry="polygon" )
